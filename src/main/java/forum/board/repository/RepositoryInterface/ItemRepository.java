...
public interface ItemRepository {
    ...
    List<Item> findAll();
    List<Item> findByWriter(String name,int start,int pageSize); // search 에 사용될 메서드( 작성자로 게시글 검색)
    List<Item> findByTitle(String title,int start,int pageSize); // search 에 사용될 메서드 (제목으로 게시글 검색)
    List<Item> findByKeyword(String keyword,int start,int pageSize); // search 에 사용될 메서드 (제목 + 게시글 검색)
